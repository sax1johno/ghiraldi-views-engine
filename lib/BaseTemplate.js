...
Template.prototype.render = function(locals, fn) {
    ...
    logger.log('trace', 'cached template string = ' + util.inspect(this.cachedTemplateString));
    
    if (this.cachedTemplateString === '') {
        this.load(this.uri);
    }

    logger.log('trace', 'locals = ' + util.inspect(locals));
    
    if (_.isNull(this.compileTemplateFunction || _.isUndefined(this.compileTemplateFunction))) {
    ...
    if (!_.isUndefined(fn) && !_.isNull(fn) && _.isFunction(fn)) {
        fn(compiledTemplate2);
    }
    return compiledTemplate2;
};
